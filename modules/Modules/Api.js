const ApiPrototype = require('../../web/ApiPrototype');
...
class Api extends ApiPrototype {
   ...
   */
  getModuleList() {
    return fsPromises.readdir(`${process.cwd()}/modules`);
  }
  ...
  isModuleApiControllerExist(sModuleName) {
    return fsPromises.access(`${process.cwd()}/modules/${sModuleName}/Api.js`).then(()=>{
      return true;
    }).catch(() => {
      return false;
    });
  ...
  isModuleSubscriberControllerExist(sModuleName) {
    return fsPromises.access(`${process.cwd()}/modules/${sModuleName}/Subscriber.js`).then(()=>{
      return true;
    }).catch(() => {
      return false;
    });
  ...
  isModuleSenderControllerExist(sModuleName) {
    return fsPromises.access(`${process.cwd()}/modules/${sModuleName}/Sender.js`).then(()=>{
      return true;
    }).catch(() => {
      return false;
    });
